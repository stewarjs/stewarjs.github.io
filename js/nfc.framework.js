(function() {
	...
	if(document.querySelectorAll('.floating-label') != -1) {
		try {
		  document.querySelector(':placeholder-shown');
		}
		catch(error) {
			// :placeholder-shown not supported (IE 10 supports but not Edge)
			var inputs = document.querySelectorAll('.floating-label input');
			for(var i = 0; i < inputs.length; i++) {
				// Is there already a value?
				inputs[i].classList[!inputs[i].value ? 'remove' : 'add']('placeholder--hidden');
				inputs[i].addEventListener('keyup', function() {
					// check if user entered a value
					this.classList[!this.value ? 'remove' : 'add']('placeholder--hidden');
				});
			}
...
( function( window, document )
{
    ...
    if( isLocalStorage && localStorage.getItem( 'nfc__frameworkRev' ) == revision )
    {
        data = localStorage.getItem( 'nfc__framework' );
        if( data )
        {
            insert();
            return true;
    ...
    try
    {
        ...
        request.onload = function()
        {
            if( request.status >= 200 && request.status < 400 )
            {
                ...
                if( isLocalStorage )
                {
                    localStorage.setItem( 'nfc__framework',  data );
                    localStorage.setItem( 'nfc__frameworkRev',   revision );
                }
            }
        }
        request.send();
...
window.onload = function() {
	...
	$('body').on('keydown', function(event) {
		if(event.keyCode == 27 && document.querySelectorAll('.menu__item[aria-expanded="true"]').length > 0) {
			menuHandler(document.querySelectorAll('.menu__item[aria-expanded="true"]')[0]);
		}
	});
	$(document).on('click', function(ev) {
		var target = ev.target;
		if(document.querySelectorAll('.menu__item[aria-expanded="true"]').length > 0  && !$(target).hasParent('.menu__container')) {
			menuHandler(document.querySelectorAll('.menu__item[aria-expanded="true"]')[0]);
		}
	});
};
...
var icons = {
	'cross': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M14.35 14.85c-0.47 0.47-1.23 0.47-1.7 0L10 11.82l-2.65 3.03c-0.47 0.47-1.23 0.47-1.7 0 -0.47-0.47-0.47-1.23 0-1.7l2.76-3.15L5.65 6.85c-0.47-0.47-0.47-1.23 0-1.7s1.23-0.47 1.7 0L10 8.18l2.65-3.03c0.47-0.47 1.23-0.47 1.7 0s0.47 1.23 0 1.7l-2.76 3.15 2.76 3.15C14.82 13.62 14.82 14.38 14.35 14.85z"/></svg>',
	'info': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M10 0.4c-5.3 0-9.6 4.3-9.6 9.6 0 5.3 4.3 9.6 9.6 9.6 5.3 0 9.6-4.3 9.6-9.6C19.6 4.7 15.3 0.4 10 0.4zM10.9 3.87c0.94 0 1.21 0.54 1.21 1.16 0 0.78-0.62 1.49-1.68 1.49 -0.89 0-1.31-0.44-1.28-1.18C9.15 4.72 9.66 3.87 10.9 3.87zM8.5 15.75c-0.64 0-1.11-0.39-0.66-2.09l0.73-3.02c0.13-0.48 0.15-0.68 0-0.68 -0.19 0-1.02 0.33-1.51 0.66L6.74 10.09c1.56-1.3 3.34-2.06 4.11-2.06 0.64 0 0.75 0.76 0.43 1.92l-0.84 3.18c-0.15 0.56-0.08 0.76 0.06 0.76 0.19 0 0.82-0.23 1.44-0.72l0.36 0.49C10.79 15.17 9.14 15.75 8.5 15.75z"/></svg>',
	'warning': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M19.51 17.98L10.6 1.35C10.48 1.13 10.25 1 10 1 9.75 1 9.52 1.13 9.4 1.35L0.49 17.98c-0.12 0.21-0.12 0.47 0.01 0.68C0.62 18.87 0.85 19 1.09 19h17.81c0.25 0 0.47-0.13 0.6-0.34C19.63 18.45 19.63 18.19 19.51 17.98zM11 17H9v-2h2V17zM11 13.5H9V7h2V13.5z"/></svg>',
	'error': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M18.69 12.34l-1.73-1.54c-0.5-0.44-0.5-1.17 0-1.62l1.73-1.54c0.5-0.44 0.37-0.86-0.3-0.92l-2.29-0.22c-0.66-0.06-1.02-0.63-0.79-1.27l1.22-3.38c0.23-0.63-0.05-0.86-0.62-0.51l-2.7 1.67c-0.57 0.35-1.3 0.16-1.63-0.43l-0.98-1.75c-0.33-0.59-0.87-0.59-1.21-0.01l-0.91 1.58C8.15 3 7.38 3.25 6.77 2.97l-1.6-0.73c-0.61-0.28-1.07 0.04-1.04 0.72l0.09 1.62c0.04 0.67-0.46 1.37-1.1 1.54L1.57 6.52c-0.64 0.17-0.76 0.68-0.26 1.13l1.73 1.54c0.5 0.45 0.5 1.17 0 1.62L1.31 12.34c-0.5 0.45-0.37 0.88 0.29 0.96l2.1 0.25c0.66 0.08 1.03 0.67 0.82 1.31l-1.07 3.26c-0.21 0.64 0.09 0.89 0.67 0.56l2.46-1.43c0.58-0.33 1.36-0.15 1.73 0.41L9.32 19.2c0.37 0.56 0.93 0.53 1.24-0.07l0.9-1.73c0.31-0.6 1.05-0.84 1.65-0.54l1.73 0.87c0.6 0.3 1.06-0.01 1.02-0.68l-0.09-1.62c-0.04-0.67 0.46-1.36 1.11-1.54l1.54-0.42C19.07 13.3 19.19 12.79 18.69 12.34zM11 14H9v-2h2V14zM11 11H9V6h2V11z"/></svg>',
	'check': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M8.294,16.998c-0.435,0-0.847-0.203-1.111-0.553L3.61,11.724c-0.465-0.613-0.344-1.486,0.27-1.951	c0.615-0.467,1.488-0.344,1.953,0.27l2.351,3.104l5.911-9.492c0.407-0.652,1.267-0.852,1.921-0.445c0.653,0.406,0.854,1.266,0.446,1.92L9.478,16.34c-0.242,0.391-0.661,0.635-1.12,0.656C8.336,16.998,8.316,16.998,8.294,16.998z"/></svg>',
    'down-arrow': '<svg class="icon" role="presentation" viewBox="0 0 20 20"><path d="M5 6h10l-5 9L5 6z"/></svg>'
};
...
$.timeoutApp = function(url){   
    ...
    var idleTime = 0,
        finalTime = 0,
   	    idleInterval,
        time_to_reach = 870000, // When to show warning dialog. Set to 14.5 minutes. Server timeout is at 15 minutes.
        dialogTime = 30; // How long to show timeout warning dialog before force to logout. Set to 30 seconds (in seconds)
	
		timerStart(showWarningDialog, time_to_reach);
...
// TOOLTIP PUBLIC CLASS DEFINITION
  ...
  var Tooltip = function (element, options) {
    ...
    this.$element   = null
    this.inState    = null
	this.template	= '<div tabindex="-1" class="tooltip" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>'
	this.viewport	= {
      selector: 'body',
      padding: 0
    }
  ...
  /*Tooltip.VERSION  = '3.3.7
  	Base version is as noted, however, modifications have been made to remediate XSS issues*/
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    this.$viewport = this.viewport && $($.isFunction(this.viewport) ? this.viewport.call(this, this.$element) : (this.viewport.selector || this.viewport))
  ...
  Tooltip.prototype.getViewportAdjustedDelta = function (placement, pos, actualWidth, actualHeight) {
    ...
    var viewportPadding = this.viewport && this.viewport.padding || 0
	...
	initEvents : function() {
       ...
       $($this.selectBox[0].attributes).each(function() {
        ...
        $this.options.not('[disabled]').each(function() {
			...
			customOptions += '<li ' + 'data-value="' + this.value + '"' + (this.getAttribute('class') ? 'class="' + this.getAttribute('class') + '"': false) + '><a href="' + (this.getAttribute('data-href') ? this.getAttribute('data-href') + '"': '#"') + (this.getAttribute('data-target') ? ' target="' + this.getAttribute('data-target') + '"': '') + '>' + svg + '<span>' + this.innerHTML + '</span></a></li>';
            svg = '';
        });
        ...
        $this.self.on('click', function(event){
			...
			if((event.keyCode == 40 || event.keyCode == 13 || event.keyCode == 32) && $this.status != 'open') {
				toggleSelect();
			}else if(event.keyCode == 27 && $this.status == 'open') {
				toggleSelect();
				$this.self.focus();
			}
		...
		var maxWidth = function() {
			var placeholder = $this.self.outerWidth( true ),
				options = $this.optBox.outerWidth( true );
			return Math.max(placeholder, options) + 50;
		}
		...
		if($this.selectBox.attr('data-width'))  {
			...
			var width = maxWidth();
			$this.self.css({'width': width + 'px'});
			$this.optBox.css({'width': width + 'px'});
		}
...
/* Sticky element (common usage on page__header)
//	
// NOTE: This CSS to make this work is not included in the framework to allow
//		 developers the flexibilty to adjust positions and other styles as needed
//
   =================================================================================== */
...
$.fn.sticky = function(options) {
		...
		var settings = $.extend({
			...
			$element = null,
			$nextElement = null,
			$nextElement_top_margin = 0;
 
		// Define where the element begins
		this.elementTop = this.offset().top;
	
		// Set our defined elements to use in our event listener
		
		$element = this;
		$nextElement = $(settings.nextElement);
		$nextElement_top_margin = Number($nextElement.css('margin-top').replace('px',''));
		
	
		// Define the height of our element to "stick" (include margin only if the nextElement doesn't have a top margin)
		if($nextElement_top_margin == 0) {
			this.height = this.outerHeight(true);
		}else{
			this.height = this.outerHeight(false);
		}
		
		// Setup the scroll event on the $(window)
	
		$(window).on('scroll', function(){
			if($(window).scrollTop() > $element.elementTop){
				if($element.hasClass(settings.class) !== true) {
					$element.addClass(settings.class);
					$nextElement.css({'margin-top' : ($nextElement_top_margin + $element.height)  + 'px'});
				}
			}else{
				if($element.hasClass(settings.class) === true) {
					$element.removeClass(settings.class);
					$nextElement.css({'margin-top' : $nextElement_top_margin + 'px'});
				}
			}
		});
